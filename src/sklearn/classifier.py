...
from ..model.orionmsp_v15 import OrionMSPv15
...
class OrionMSPv15Classifier(ClassifierMixin, BaseEstimator):
    """

    This classifier applies ORION-MSP to tabular data classification, using an ensemble
    ...
    def __init__(
        self,
        n_estimators: int = 64,
        norm_methods: Optional[str | List[str]] = ["none", "power", "quantile"],
        ...
        model_path: Optional[str | Path] = "/path/to/checkpoint/",
        allow_auto_download: bool = False,
        checkpoint_version: str = "OrionMSP-v1.5.ckpt",
        ...
        adaptive_ensemble_weights: bool = False,
        ensemble_weight_method: str = "dataset_stats",  # "dataset_stats", "performance", "uniform"
    ):
        ...
        self.adaptive_ensemble_weights = adaptive_ensemble_weights
        self.ensemble_weight_method = ensemble_weight_method
        
        if adaptive_ensemble_weights:
            # Learn dataset-specific ensemble weights
            # This will be initialized after fit()
            self.ensemble_weight_predictor_ = None
    ...
    def _load_model(self):
        ...
        self.model_ = OrionMSPv15(**checkpoint["config"])
        self.model_.load_state_dict(checkpoint["state_dict"])
        self.model_.eval()

    def _compute_dataset_stats(self, X: np.ndarray) -> np.ndarray:
        """
        FEATURE ADDITION: Compute dataset statistics for adaptive ensemble weighting.
        
        Returns features like: mean, std, skewness, kurtosis, etc.
        """
        stats = []
        # Mean
        stats.append(X.mean(axis=0))
        # Std
        stats.append(X.std(axis=0))
        # Min/Max
        stats.append(X.min(axis=0))
        stats.append(X.max(axis=0))
        # Number of samples and features
        stats.append(np.array([X.shape[0], X.shape[1]]))
    ...
    def fit(self, X, y):
        ...
        4. Loading the pre-trained OrionMSP model
        ...
        # Initialize adaptive ensemble weights
        if self.adaptive_ensemble_weights:
            # Compute dataset statistics
            dataset_stats = self._compute_dataset_stats(X)
            self.dataset_stats_mean_ = dataset_stats.mean()
            self.dataset_stats_std_ = dataset_stats.std()
            
            # Simple MLP for weight prediction
            stats_dim = len(dataset_stats)
            self.ensemble_weight_predictor_ = nn.Sequential(
                nn.Linear(stats_dim, 32),
                nn.ReLU(),
                nn.Linear(32, self.n_estimators),
                nn.Softmax(dim=-1)
            )
    ...
    def _batch_forward(self, Xs, ys, shuffle_patterns=None):
        ...
        This method handles the batched inference through the ORION-MSP model,
    ...
    def predict_proba(self, X):
        ...
        Applies the ensemble of ORION-MSP models to make predictions, with each ensemble
        ...
        if self.n_jobs is not None:
            ...
            if self.n_jobs > 0:
                if self.n_jobs > n_logical_cores:
                    warnings.warn(
                        f"ORION-MSP got n_jobs={self.n_jobs} but there are only {n_logical_cores} logical cores available."
