...
def build_parser():
    ...
    parser.add_argument("--prior_n_jobs", type=int, default=32, help="Number of parallel jobs for prior generation (only works with prior_device='cpu')")
    ...
    parser.add_argument(
        ...
        type=str,
        default="subspace",
        choices=["subspace", "learned", "none"],
        help="Feature positional embedding method: 'subspace' (like TabPFN), 'learned', or 'none'.",
    )

    # Multi-Scale Sparse Row interactor
    parser.add_argument("--row_num_blocks", type=int, default=9)  # Increased default for single scale
    ...
    # Default to single scale
    parser.add_argument("--row_scales", type=int_list, default=[1])  # Single scale by default
    ...
    # Feature grouping parameter
    parser.add_argument(
        "--features_per_group",
        type=int,
        default=2,
        help="Number of features to group together (like TabPFN's features_per_group). Default: 2.",
    )
    parser.add_argument(
        "--scale_combine_method",
        type=str,
        default="enhanced_attention",  # Updated default
        choices=["mean", "learned_weight", "learned_attention", "enhanced_attention", "concat"],
        help="How to combine multi-scale outputs: mean, learned_weight, learned_attention, enhanced_attention (best), or concat.",
    )

    # Perceiver memory
    parser.add_argument("--perc_num_latents", type=int, default=32)  # Increased default
    parser.add_argument("--perc_layers", type=int, default=2)
    # FEATURE ADDITION: Enhanced memory parameters
    parser.add_argument(
        "--num_memory_heads",
        type=int,
        default=4,
        help="Number of memory heads for multi-head Perceiver memory. Default: 4.",
    )
    parser.add_argument(
        "--use_memory_gating",
        default=True,
        type=str2bool,
        help="Whether to use memory gating mechanism. Default: True.",
    )
    ...
    # FEATURE ADDITION: Thinking tokens
    parser.add_argument(
        "--num_thinking_tokens",
        type=int,
        default=0,
        help="Number of thinking tokens to add before test samples. Default: 0 (disabled).",
    )
